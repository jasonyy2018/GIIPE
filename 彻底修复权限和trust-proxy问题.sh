#!/bin/bash

# 彻底修复权限问题和 trust proxy 配置

set -e

# 颜色定义
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m'

log_info() { echo -e "${BLUE}[INFO]${NC} $1"; }
log_success() { echo -e "${GREEN}[SUCCESS]${NC} ✅ $1"; }
log_warning() { echo -e "${YELLOW}[WARNING]${NC} ⚠️  $1"; }
log_error() { echo -e "${RED}[ERROR]${NC} ❌ $1"; }

echo "=========================================="
echo "🔧 彻底修复权限和 trust proxy 问题"
echo "=========================================="
echo ""

# 检查是否在正确的目录
if [ ! -f "docker-compose.prod.yml" ]; then
    log_error "请在项目根目录运行此脚本"
    exit 1
fi

log_info "[1/7] 停止后端服务..."
docker compose -f docker-compose.prod.yml stop backend
log_success "后端服务已停止"

log_info "[2/7] 修复主机上传目录权限..."
# 确保主机目录存在且有正确权限
mkdir -p uploads/images uploads/documents uploads/pdfs uploads/avatars
chmod -R 777 uploads
log_success "主机目录权限已设置"

log_info "[3/7] 重新构建后端（应用 trust proxy 修复）..."
log_warning "这可能需要几分钟..."

BUILD_OUTPUT=$(docker compose -f docker-compose.prod.yml build --no-cache backend 2>&1)
BUILD_EXIT_CODE=$?

if [ $BUILD_EXIT_CODE -eq 0 ]; then
    log_success "后端构建成功"
else
    log_error "后端构建失败"
    echo "$BUILD_OUTPUT" | tail -50
    exit 1
fi

log_info "[4/7] 启动后端服务..."
docker compose -f docker-compose.prod.yml up -d backend
log_success "后端服务已启动"

log_info "[5/7] 等待后端启动..."
sleep 15

log_info "[6/7] 在运行中的容器内修复权限..."
# 在容器内修复权限
docker compose -f docker-compose.prod.yml exec -T backend sh << 'EOF'
# 确保目录存在
mkdir -p /app/uploads/images /app/uploads/documents /app/uploads/pdfs /app/uploads/avatars

# 修复权限（使用 root 权限）
chown -R nestjs:nodejs /app/uploads
chmod -R 755 /app/uploads

# 确保 images 目录可写
chmod 777 /app/uploads/images

# 验证权限
echo "=== 权限验证 ==="
ls -la /app/uploads
ls -la /app/uploads/images 2>/dev/null || echo "images 目录不存在，将创建"
EOF

log_success "容器内权限已修复"

log_info "[7/7] 测试写入权限..."
# 测试写入权限
docker compose -f docker-compose.prod.yml exec -T backend sh << 'EOF'
TEST_FILE="/app/uploads/test_write_$(date +%s).txt"
echo "test" > "$TEST_FILE" 2>&1 && rm -f "$TEST_FILE" && echo "✅ 写入权限测试成功" || echo "❌ 写入权限测试失败"
EOF

# 检查后端日志
log_info "检查后端日志（应该不再有 trust proxy 警告）..."
TRUST_PROXY_WARNINGS=$(docker compose -f docker-compose.prod.yml logs backend --tail 50 | grep -i "ERR_ERL_PERMISSIVE_TRUST_PROXY\|trust proxy" || echo "")

if [ -z "$TRUST_PROXY_WARNINGS" ]; then
    log_success "✅ 未发现 trust proxy 警告"
else
    log_warning "⚠️  仍可能有 trust proxy 警告，请检查日志"
    echo "$TRUST_PROXY_WARNINGS" | head -3
fi

# 检查权限错误
log_info "检查权限错误..."
PERMISSION_ERRORS=$(docker compose -f docker-compose.prod.yml logs backend --tail 50 | grep -i "EACCES\|permission denied" || echo "")

if [ -z "$PERMISSION_ERRORS" ]; then
    log_success "✅ 未发现权限错误"
else
    log_warning "⚠️  仍可能有权限错误，请检查日志"
    echo "$PERMISSION_ERRORS" | head -3
fi

# 检查服务状态
log_info "检查服务状态..."
docker compose -f docker-compose.prod.yml ps backend

echo ""
echo "=========================================="
log_success "修复完成！"
echo "=========================================="
echo ""
echo "📋 修复内容："
echo "1. ✅ Trust proxy 配置更安全（只信任第一个代理）"
echo "2. ✅ 上传目录权限已修复"
echo ""
echo "📋 验证步骤："
echo "1. 检查后端日志，应该不再有 trust proxy 警告："
echo "   docker compose -f docker-compose.prod.yml logs backend --tail 50 | grep -i 'trust proxy'"
echo ""
echo "2. 检查权限错误："
echo "   docker compose -f docker-compose.prod.yml logs backend --tail 50 | grep -i 'EACCES\|permission'"
echo ""
echo "3. 测试文件上传功能"
echo ""

